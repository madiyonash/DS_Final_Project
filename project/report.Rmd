...
In the `report_2019_sep` data, we have a simpler dataframe. This only includes the date of the observation and the average mood for that particular day. There are 365 observations of this dataset for the 365 days in the year. Let's take a look at it:
```{r}
#print sample of this report
report_2019_day %>% 
  sample_n(5)
...
retrograde_mood %>% 
  filter(isRetrograde == T) 
```
We can also graph each of these retrogrades:
```{r}
retrograde_mood %>% 
  mutate(week1 = 1 if filter(between(date, as.Date("2019-03-07"), as.Date("2019-03-29")) == T),
         week2 = filter(between(date, as.Date("2019-07-09"), as.Date("2019-08-01"))),
         week3 = filter(between(date, as.Date("2019-11-02"), as.Date("2019-11-21"))))
...
### Does the school year affect the activities performed?

Earlier, we found that homework was the most frequent activity. Let's see if we can find the approximate time that summer is based on this.
```{r}
report_2019_activities %>% 
  filter(activities == c("homework", "test")) %>% 
  ggplot() +
  geom_point(aes(x = date, y = activities))
```
# Conclusion
